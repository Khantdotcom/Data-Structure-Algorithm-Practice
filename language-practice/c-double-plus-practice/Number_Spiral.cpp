...
void solve() {
    ...
    for (int i =0;i<n;i++){
        ...
        cin >> x >> y;
        ll max_num = max(x,y);
        ll grind_size = max_num * max_num;

        if ((max_num & 1) == 0){
            grind_size -= abs(max_num-x) + abs(1-y);
        }else{
            grind_size -= abs(1-x) + abs(max_num-y);
        }

        // for odd grid, 1 x is grid size;
        // for even gird, x 1 is grid size;
        cout << grind_size << endl;
    }
}
